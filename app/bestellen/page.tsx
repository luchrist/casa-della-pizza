...
/* Category indices for suggestions */
const DESSERT_CAT = "Dolci";
const DRINK_CAT = "Alkoholfreie Getränke";
...
const MAIN_CATS = new Set([
  ...
  "Pizze di Mare",
  "Pizze Vegetariane",
  "Pikante & Fleisch-Pizzen",
  "Calzone & Party",
  "Pasta",
  "Schnitzel",
]);

/* Hand-picked bestsellers across categories */
const BELIEBTESTE_NAMES = new Set([
  "Pizza Margherita",
  "Pizza Mista",
  "Pizza Diavola",
  "Pizza Parma",
...
type MenuCategory = { category: string; subtitle: string; items: { name: string; description: string; price: string }[] };
...
export default function BestellenPage() {
  const beliebteste: MenuCategory = useMemo(() => {
    const items: MenuCategory["items"] = [];
    for (const cat of config.menu) {
      for (const item of cat.items) {
        if (BELIEBTESTE_NAMES.has(item.name)) items.push(item);
      }
    }
    return { category: "Beliebteste", subtitle: "Das wird am meisten bestellt", items };
  }, []);

  const categories: MenuCategory[] = useMemo(
    () => [beliebteste, ...config.menu],
    [beliebteste]
  );
  ...
  const addToCart = useCallback(
    (
      ...
      if (showSuggestion && MAIN_CATS.has(category)) {
        const hasDrink = cart.some((i) => i.category === DRINK_CAT);
        const hasDessert = cart.some((i) => i.category === DESSERT_CAT);

        if (!hasDrink) {
          const drinks = categories.find((c) => c.category === DRINK_CAT);
          if (drinks && drinks.items.length > 0) {
            const pick =
              drinks.items[Math.floor(Math.random() * drinks.items.length)];
            setSuggestion({ type: "drink", item: pick });
            return;
          }
        }
        if (!hasDessert) {
          const desserts = categories.find((c) => c.category === DESSERT_CAT);
          if (desserts && desserts.items.length > 0) {
            const pick =
              desserts.items[
                Math.floor(Math.random() * desserts.items.length)
              ];
              ...
              <button
                onClick={() => {
                  addToCart(
                    ...
                    suggestion.type === "drink" ? DRINK_CAT : DESSERT_CAT,
                    false
                  );
                  setSuggestion(null);
                }}
